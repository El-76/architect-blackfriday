...
async def startup():
    if REDIS_URL:
        redis = RedisCluster.from_url(REDIS_URL, encoding="utf8", decode_responses=True)

        original_pipeline = redis.pipeline
        def cluster_pipeline_patched(*args, **kwargs):
            kwargs["transaction"] = False
            return original_pipeline(*args, **kwargs)
        
        redis.pipeline = cluster_pipeline_patched
...
async def root():
    ...
    for collection_name in collection_names:
        ...
        if "shards" in stats:
            for shard_name, shard_info in stats["shards"].items():
                collections[collection_name][shard_name] = shard_info['count']
    ...
    if topology_type == "Sharded":
        ...
        for shard in shards_list.get("shards", {}):
            ...
            for host in hosts.split(','):
                direct_client = motor.motor_asyncio.AsyncIOMotorClient(f"mongodb://{host}/?directConnection=true", serverSelectionTimeoutMS=2000)

                hello = await direct_client.admin.command("hello")
    ...
    if REDIS_URL:
        ...
        redis_nodes = await FastAPICache.get_backend().redis.cluster_nodes()
